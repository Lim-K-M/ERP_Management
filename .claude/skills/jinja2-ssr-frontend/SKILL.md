...
├── login.html                # 로그인 폼 (스펙 범위 밖 별도 확장 — app/core/auth.py 참고)
├── partials/
...
└── employees/
    ...
    ├── register.html
    └── detail.html            # 인사발령 이력 5건 초과 시 페이지네이션 포함
```
...
- CSRF 토큰은 **도입하지 않는다** — 스펙 §7은 "인증 없음, 배포하지 않는 로컬 데모"를 전제로 이 트레이드오프를 허용했다.
- **재검토 결과(로그인 확장 이후)**: 이 프로젝트는 이후 스펙 범위 밖 확장으로 세션 로그인을 추가해 실제 인증 컨텍스트가 생겼다 — 위 트레이드오프의 전제가 더 이상 100% 성립하지 않는다. 재검토했지만 Starlette `SessionMiddleware`의 기본 쿠키 옵션(`SameSite=Lax`)이 일반적인 크로스사이트 POST 위조를 상당 부분 막아주는 점, 로컬 데모 범위를 벗어나지 않는 점을 근거로 CSRF 토큰은 여전히 도입하지 않기로 했다. 잔여 리스크는 `docs/internal/progress-checklist.md`의 "잔여 리스크" 절에 기록되어 있다 — 실제 배포로 전환하면 이 판단부터 다시 검토해야 한다.
