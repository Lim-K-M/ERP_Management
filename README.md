...
- `employee_service.py`에 `count_by_status`/`count_by_department`/`count_by_position` GROUP BY 쿼리 3개 추가 — 최초 구현 시점에는 전체(global) 기준으로 집계했다(이후 PR #31에서 사이드바 필터 반영으로 변경)
...
### PR #28 반영 문서 정합성 정리 + 스크린샷 재촬영 (2026-07-13, PR #29)

PR #28의 여러 차례 UI 변경(브랜드 배너 인원 수 표시, 컨테이너 폭 확장)을 반영해 `README.md`/`progress-checklist.md`/`ai-usage.md`의 PR 개수·목록을 정정하고, 결과화면 스크린샷 8장을 동일 시나리오로 재촬영했다.

### 목록 입사일 컬럼 정렬 기능 추가 (2026-07-13, PR #30)

사용자가 "입사일에 오름차순/내림차순 버튼이 없다"고 직접 지적해 발견한 누락 기능. 다른 컬럼(사번/이름/부서/직급/상태)엔 전부 정렬 화살표가 있었는데 입사일만 빠져 있었다.

- `employee_service.py`의 `SORT_COLUMNS`에 `hire_date` 추가, `list.html` 헤더에 동일한 정렬 화살표 UI 추가
- `api_employees.py`는 같은 `SORT_COLUMNS`/`DEFAULT_SORT`를 참조하므로 JSON API에도 자동 반영
- **실제 DB 연동 검증 완료**: `?sort=hire_date&order=asc` 응답이 실제로 2018-02-09부터 오름차순 정렬되는 것을 curl/Playwright로 확인

### 사이드바 필터 적용 시 인원 수 재계산 (2026-07-13, PR #31)

PR #28에서 추가한 부서/직급/상태별 인원 수가 항상 전체(global) 기준으로 집계돼, 사이드바에서 조건을 걸어도 배너의 숫자가 바뀌지 않던 것을 수정.

- `employee_service.py`의 `count_by_status`/`count_by_department`/`count_by_position`이 `filters: EmployeeFilter | None`을 받아 목록 조회와 동일한 `_apply_filters()`를 거치도록 변경
- `pages.py`에서 이미 구성해둔 `filters` 객체를 세 카운트 함수에 그대로 전달
- **실제 DB 연동 검증 완료**: `dept_id=1` 필터 시 직급별 분포가 전체(16/18/23/23/20) → 해당 부서만(3/8/5/5/2)으로 좁혀지는 것, `status=ACTIVE` 필터 시 휴직/퇴직 카운트가 0으로 떨어지는 것을 curl 응답으로 확인
- 서버가 `--reload` 없이 기동 중이어서 코드 변경이 반영되지 않던 것을 재확인(기존에도 겪었던 이슈) — 재시작 후 검증
- 부서 5개가 한 줄에 안 들어가 배너가 2줄로 접히던 문제를 페이지 컨테이너 `max-width`를 1040px → 1180px로 소폭 확장해 해결

### 내부 정리 — 죽은 CSS 제거, 문서 drift 수정, 중복 체크리스트 통합 (2026-07-14, PR #32)

UI가 아닌 내부(코드 위생/문서 정합성) 관점에서 점검한 결과 정리:

- `style.css`의 어떤 템플릿에서도 참조하지 않는 `.toolbar` 규칙 제거(사이드바 필터 UI로 바뀌며 남은 잔재로 추정)
- `progress-checklist.md`에 PR #31 이후에도 남아있던 "카운트는 전체 기준이라 필터를 걸어도 숫자가 안 바뀐다"는 stale한 문구 정정, PR 개수(28→31) 및 최종 PR 목록/브랜치 표에 #29~#31 반영, `ai-usage.md` PR 개수 동기화
- `docs/internal/2026-07-12-requirements-checklist.md`가 `progress-checklist.md`의 "요구사항 정의서 대조 결과" 절과 내용이 거의 전부 겹쳐 있던 것을 확인 후, 더 상세한 쪽(§1~§7 세부 표 + 결론)을 `progress-checklist.md`로 흡수하고 별도 파일은 삭제
- 레포 루트에 개인적으로 다운받아 두던 제출용 PDF 2개가 미추적 상태로 계속 남아있던 것을 `.gitignore`에 `/*.pdf` 패턴 추가로 정리

### 사번 입력 placeholder 예시 수정 (2026-07-14, PR #33)

등록 폼의 사번 placeholder가 "예: 0001"로 남아있었는데, 실제 시드 데이터/컨벤션은 1001~1100(1XXX)이라 사용자가 직접 발견해 지적. placeholder만 "예: 1001"로 수정(검증 로직은 이 시점까지 아직 `^\d{4}$`라 동작 변경은 없었음).

### 사번은 1로 시작하는 숫자 4자리만 허용 (2026-07-14, PR #34)

사용자가 "사번이 0으로 시작하면 저장이 안 되게 해달라"고 요청해 `EMP_NO_PATTERN`을 `^\d{4}$` → `^[1-9]\d{3}$`로 좁혔으나, 곧바로 "0 금지가 아니라 무조건 1로 시작"이 맞는 규칙이라는 정정을 받아 최종적으로 `^1\d{3}$`(1로 시작하는 숫자 4자리)로 재확정했다. `EmployeeCreate`/`EmployeeUpdate`가 같은 `_normalize_emp_no()`를 공유하므로 등록/수정/API 전부에 자동 반영됨.

- **실제 DB 연동 검증 완료**: 로그인 세션 + Playwright로 등록 폼에서 `0111`(422, DB 미저장 확인) / `9111`→규칙 확정 전 임시 검증(303, 저장 확인 후 삭제) / 최종 규칙 확정 후 인증된 JSON API로 `0999`·`2999`(422) / `1999`(201, 확인 후 삭제) 재검증

### 규칙/스킬/계획 재점검 (2026-07-14, PR #35)

"UI 말고 내부에서 보완할 부분이 있을까"라는 요청에 이어, 범위를 `.claude/rules`·`.claude/skills`·`docs/plans`까지 넓혀 다시 점검:

- `.claude/skills/docker-compose-server-ops`가 초기 커밋 때부터 `.claude/rules/development-workflow.md`·`.claude/skills/README.md`·`code-review-standard/SKILL.md` 등 5개 파일 8곳에서 계속 참조돼 왔지만, 실제로는 `git log`로 확인해도 이 폴더가 **생성된 이력 자체가 없는** 것을 발견(스펙 §7상 배포를 하지 않아 실제로 필요했던 적이 없었던 것으로 추정). 참조를 삭제하지 않고 전부 "(미사용/미작성)"으로 표시하고, `README.md`(스킬 안내)의 "미사용 스킬" 부록에 근거를 남겼다
- PR #34로 `emp_no` 정규식이 `^1\d{3}$`으로 바뀐 뒤에도 옛 패턴(`^\d{4}$`/`^A\d{4}$`)을 현재 상태인 것처럼 적어둔 문서 4곳(`fastapi-service-architecture/SKILL.md`, `2026-07-08-employee-erd.md`, `progress-checklist.md` 2곳) 정정
- 이 README 체인지로그에 PR #29·#30·#32~#34가 아예 누락돼 있던 것을 발견해 함께 보완
