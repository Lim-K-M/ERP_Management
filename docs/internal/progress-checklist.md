...
- [x] 커밋을 의미 단위로 나눴다 — 전체 35개 PR로 기능/문서 단위 분리(아래 "최종 PR 목록" 참고). 중간에 실수로 `main`에 직접 커밋한 1건은 즉시 브랜치로 옮기고 `main`을 원복해 규칙을 회복했다.
...
| `docs/internal-cleanup-and-doc-consolidation` | 내부 정리 2차 — 죽은 CSS 제거, 문서 drift 정정, 중복 체크리스트 통합 | 완료, main 병합 |
| `fix/emp-no-placeholder` | 등록 폼 사번 placeholder를 실제 컨벤션(1XXX)에 맞게 수정 | 완료, main 병합 |
| `fix/emp-no-no-leading-zero` | 사번을 1로 시작하는 숫자 4자리로 제한(`^1\d{3}$`) | 완료, main 병합 |
| `docs/rules-skills-plans-recheck` | 규칙/스킬/계획 재점검 — `docker-compose-server-ops`(실체 없는 참조) 미사용 표시, emp_no 관련 문서 drift 정정 | 완료, main 병합 |
...
| 필드 | 스펙 요구사항 | 상태 |
|---|---|---|
| `emp_no` | 필수·최대 20자·고유 | ✅ (⚠️ 스펙 초과: 1로 시작하는 숫자 4자리 형식(`^1\d{3}$`) 강제 — 별도 확장, 변경 이력은 아래 "스펙 범위 밖 별도 확장" 참고) |
| `emp_name` | 필수·최대 100자 | ✅ |
...
- **사번/이메일/전화번호 형식 검증 강화**: `docs/specs/` §4는 `emp_no`를 "필수, 최대 20자, 고유", `phone`을 "선택, 최대 20자"로만 규정하고 패턴을 요구하지 않는다. 사용자 요청에 따라 `emp_no`는 `^1\d{3}$`(1로 시작하는 숫자 4자리, 예: `1001`), `phone`은 하이픈 없는 숫자 9~11자리로 스펙보다 엄격하게 제한했다(`app/schemas/employee.py`). **주의**: 마감 제출물을 스펙 §4 기준으로만 평가할 경우, 이 형식과 다른 사번/전화번호 테스트 데이터는 422로 거부된다 — 스펙에 없는 제약이니 평가자에게 별도 확장임을 설명하거나 필요 시 완화 검토.
  - **변경 이력**: `^A\d{4}$`(알파벳 A 고정, PR #15) → "알파벳 상관없이 숫자 4자리로만"으로 재확정한 `^\d{4}$`(PR #23) → 사용자가 "0으로 시작하면 안 되는 게 아니라 무조건 1로 시작"이라고 재정정해 `^1\d{3}$`(PR #34)로 최종 확정.
...
## 최종 PR 목록 (2026-07-14 기준, 총 35개 · 전부 main 병합 완료)
...
| #32 | chore(docs): 내부 정리 — 죽은 CSS 제거, 문서 drift 수정, 중복 체크리스트 통합 | 품질 개선 |
| #33 | fix(employee): 사번 입력 placeholder 예시를 실제 컨벤션(1XXX)에 맞게 수정 | 버그 수정 |
| #34 | fix(employee): 사번은 1로 시작하는 숫자 4자리만 허용 | 버그 수정 |
| #35 | chore(docs): 규칙/스킬/문서 재점검 — docker-compose-server-ops 미사용 표시 + emp_no 문서 drift 정정 | 품질 개선 |
...
- 등록 폼의 사번 placeholder가 실제 컨벤션(1XXX)과 다른 "0001"로 남아있던 것을 사용자가 직접 발견해 수정(#33). 이어서 "사번이 0으로 시작하면 안 된다"는 요청으로 `^[1-9]\d{3}$`을 적용했으나, 곧바로 "0 금지가 아니라 무조건 1로 시작"이 맞는 규칙이라는 정정을 받아 `^1\d{3}$`으로 최종 확정(#34).
- 내부 정리(#32) 이후 재점검 요청에 따라 `.claude/rules`·`.claude/skills`까지 범위를 넓혀 다시 확인한 결과, 처음부터 실제로 작성된 적 없는 `docker-compose-server-ops` 스킬(폴더 자체가 생성된 이력이 없음, `git log`로 확인)에 대한 참조가 5개 파일 8곳에 남아있던 것을 발견. 사용자 지시대로 삭제 대신 "(미사용/미작성)"으로 표시하고 `.claude/skills/README.md` 부록에 근거를 남겼다(#35). 같은 PR에서 `emp_no` 정규식이 `^1\d{3}$`으로 바뀐 뒤에도 여러 문서(스킬 문서·ERD·체크리스트)가 옛 패턴(`^\d{4}$`/`^A\d{4}$`)을 현재 상태인 것처럼 적어둔 것, README 체인지로그에 PR #32~#34가 아예 누락됐던 것도 함께 정정했다.
